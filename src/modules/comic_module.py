...
def daily_comic():
    ...
    if request.method == 'POST':
        ...
        if not check_and_deduct_points(user_id, 'daily_news_comic'):
            ...
            return redirect(url_for('comic.daily_comic'))
...
def custom_comic():
    if request.method == 'POST':
        ...
        if not check_and_deduct_points(user_id, 'custom_comic'):
            ...
            return redirect(url_for('comic.custom_comic'))
...
def media_comic():
    if request.method == 'POST':
        ...
        if not check_and_deduct_points(user_id, 'media_comic'):
            ...
            return redirect(url_for('comic.media_comic'))
            ...
            for event_index, event in enumerate(generated_comics):
                ...
                if isinstance(event, dict):
                    ...
                    if image_paths:
                        ...
                        event['image_paths'] = [url_for('media.serve_image', filename=os.path.relpath(path, current_app.config['GENERATED_IMAGES_FOLDER']), _external=True) for path in image_paths]
            ...
            if 'image_path' in comic and comic['image_path']:
                ...
                for path in image_paths:
                    if not os.path.isabs(path):
                        path = os.path.join(current_app.config['GENERATED_IMAGES_FOLDER'], path)
